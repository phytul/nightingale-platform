...
class CaptchaStorage:
    ...
    async def store_email_code(self, email: str, code: str, scene: str = "register") -> bool:
        """
        存储邮箱验证码
    ...
    async def verify_email_code(self, email: str, code: str, scene: str = "register") -> Dict[str, Any]:
        """验证邮箱验证码"""
        try:
            key = f"captcha:email:{scene}:{email}"
            ...
            return {"success": False, "message": f"验证过程出现异常:{e}"}